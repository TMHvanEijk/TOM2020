...
def isStartWithAorB(word_count_pair):
    word, count = word_count_pair
    word_lower = word.lower()
    if word_lower.startswith('a') or word_lower.startswith('b'):
        return True
    return False
...
def run(argv=None, save_main_session=True):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        output2 = (counts
                   | 'FilterStartWithAorB' >> beam.Filter(isStartWithAorB)
                   | 'MapStartWithAorB' >> beam.Map(startWithAorB)
